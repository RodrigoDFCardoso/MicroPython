...
buzzer1 = PWM(Pin(21))
buzzer1.freq(50)  # Frequência inicial grave
buzzer2 = PWM(Pin(8))
...
def gradual_light_sound(duration=2):
    ...
    for i in range(101):
        ...
        buzzer1.freq(50 + i * 2)  # Ajustar a frequência do buzzer
        
        # Ajustar o volume do buzzer
        buzzer1.duty_u16(duty_cycle // 4)  # Reduzir o volume para 25% do máximo
        
        time.sleep(step_duration)
        
        # Ajustar a frequência do buzzer
        buzzer2.freq(50 + i * 2)  # Ajustar a frequência do buzzer
        
        # Ajustar o volume do buzzer
        buzzer2.duty_u16(duty_cycle // 4)  # Reduzir o volume para 25% do máximo
...
def beep(freq=1000, duration=0.2):
    ...
    buzzer1.freq(freq)
    buzzer1.duty_u16(5000)  # Intensidade média
    time.sleep(duration)
    buzzer1.duty_u16(0)  # Desliga o buzzer
    
    buzzer2.freq(freq)
    buzzer2.duty_u16(5000)  # Intensidade média
    time.sleep(duration)
    buzzer2.duty_u16(0)  # Desliga o buzzer
    
def star_trek_beep():
    #buzzer1 = PWM(Pin(21))  # Buzzer A conectado ao GPIO21
    
    # Primeiro tom
    buzzer1.freq(1000)
    buzzer1.duty_u16(40000)
    time.sleep(0.1)
    buzzer1.duty_u16(0)
...
def l3_37_sound_and_lights():
    ...
    for freq in [440, 392, 440, 392]:  # Notas: A, G
        ...
        np.write()
        buzzer1.freq(freq)
        buzzer1.duty_u16(5000)
        time.sleep(0.1)
        buzzer2.freq(freq)
        buzzer2.duty_u16(5000)
        time.sleep(0.1)
...
def xplosion():
    ...
    for freq in range(2000, 50, -50):
        buzzer1.freq(freq)
        buzzer1.duty_u16(32767)  # 50% de duty cycle
        time.sleep(0.005)
    
    for freq in range(2000, 50, -50):
        buzzer2.freq(freq)
        buzzer2.duty_u16(32767)  # 50% de duty cycle
        time.sleep(0.005)
    ...
    for _ in range(5):
        buzzer1.duty_u16(16383)  # 25% de duty cycle
        time.sleep(0.1)
        buzzer1.duty_u16(0)  # Desliga
        time.sleep(0.1)

    buzzer1.duty_u16(0)  # Certifique-se de que o buzzer esteja desligado

    # Reverberações ou ecos mais suaves
    for _ in range(5):
        buzzer2.duty_u16(16383)  # 25% de duty cycle
        time.sleep(0.1)
        buzzer2.duty_u16(0)  # Desliga
        time.sleep(0.1)

    buzzer2.duty_u16(0)  # Certifique-se de que o buzzer esteja desligado
